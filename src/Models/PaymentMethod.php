...
class PaymentMethod extends Model
{
    ...
    public static function getProviderConfig(string $provider): ?array
    {
        ...
        switch ($provider) {
            case self::STRIPE:
                return ['stripe.id' => $paymentMethod->id, 'stripe.id' => $paymentMethod->id, 'stripe.key' => $paymentMethod->configs['API_KEY'], 'stripe.secret' => $paymentMethod->configs['API_SECRET'], 'stripe.webhook.secret' => $paymentMethod->configs['WEBHOOK_SECRET'], 'stripe.enabled' => $paymentMethod->active, 'stripe.supported_currencies' => $paymentMethod->supported_currencies];
            case self::PAYPAL:
            ...
            case self::PAYU:
                return ['payu.id' => $paymentMethod->id, 'payu.merchant_key' => $paymentMethod->configs['MERCHANT_KEY'] ?? null, 'payu.merchant_salt' => $paymentMethod->configs['MERCHANT_SALT'] ?? null, 'payu.test_mode' => $paymentMethod->test_mode, 'payu.webhook_url' => $paymentMethod->webhook, 'payu.enabled' => $paymentMethod->active, 'payu.supported_currencies' => $paymentMethod->supported_currencies];
            default:
                return null;
        }
    }
