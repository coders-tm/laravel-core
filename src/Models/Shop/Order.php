...
class Order extends Model implements Currencyable
{
    ...
    protected $fillable = ['customer_id', 'orderable_id', 'orderable_type', 'billing_address', 'shipping_address', 'note', 'collect_tax', 'source', 'sub_total', 'shipping_total', 'tax_total', 'discount_total', 'grand_total', 'paid_total', 'refund_total', 'line_items_quantity', 'due_date', 'status', 'payment_status', 'fulfillment_status', 'shipped_at', 'delivered_at', 'tracking_number', 'tracking_company', 'cancelled_at', 'metadata'];
    ...
    public function syncLineItems(Collection $line_items, $detach = true)
    {
        ...
        foreach ($line_items as $item) {
            $product = $this->line_items()->updateOrCreate(['id' => has($item)->id], Arr::only($item, (new Coderstm::$orderLineItemModel)->getFillable()));
    ...
    public function duplicate()
    {
        ...
        $replicate = $this->loadMissing(['line_items', 'tax_lines', 'discount'])->replicate(['number', 'created_at', 'updated_at', 'due_date', 'status', 'payment_status', 'fulfillment_status', 'paid_total', 'refund_total'])->toArray();
