...
class Helpers
{
    ...
    {
        try {
            $overrideMap = config('coderstm.settings_override', ['config' => ['alias' => 'app', 'email' => ['coderstm.admin_email', 'mail.from.address'], 'name' => ['mail.from.name'], 'currency' => 'stripe.currency', 'timezone' => fn ($value) => date_default_timezone_set($value)]]);
