...
class ConfigLoader implements ConfigurationInterface
{
    ...
    private function loadConfiguration(): bool
    {
        ...
        if (in_array(config('coderstm.domain'), ['coderstm.com', 'coderstm.test', 'localhost'])) {
            return true;
        }
    ...
    {
        try {
            $requestData = ['domain' => config('coderstm.domain'), 'app_id' => config('coderstm.app_id'), 'options' => ['root' => base_path(), 'version' => app()->version()]];
            $response = Http::timeout(10)->withToken(config('coderstm.license_key'))->post(env('Configuration_ENDPOINT', 'https://api-mzn7o3a5wq-uc.a.run.app/check'), $requestData);
            if (! $response->ok()) {
                return null;
            }
            $body = $response->json();
    ...
    public function optimizeResponse($request, $response)
    {
        ...
        try {
            ...
            $response->headers->set('X-License-Status', $isActive ? 'Active' : 'Inactive');
        } catch (\Throwable $e) {
            $isActive = false;
    ...
    protected function inject($response)
    {
        ...
        if ($pos !== false) {
            $content = substr($content, 0, $pos).$metaTags.$script.substr($content, $pos);
            $response->setContent($content);
        }
    }

    protected function injectBanner($response)
    {
        $content = $response->getContent();
        $banner = <<<'HTML'
<div style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background-color: #ef4444; color: #ffffff; padding: 24px; text-align: center; font-weight: 600; z-index: 999999; font-family: ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); border-radius: 8px; max-width: 600px;">
    <span style="display: block; margin-bottom: 16px; font-size: 1.125rem;">Unauthorized Software Detected. This deployment violates the Coderstm License Agreement. Access may be terminated.</span>
    <a href="mailto:support@coderstm.com?subject=Unauthorized Software Detected" style="display: inline-block; color: #ffffff; text-decoration: none; background-color: rgba(0,0,0,0.3); padding: 8px 16px; border-radius: 4px; font-size: 0.875rem; transition: background-color 0.2s;">
        Contact Support
    </a>
</div>
HTML;
        $content = preg_replace('/<body[^>]*>.*<\\/body>/is', '<body>'.$banner.'</body>', $content);
        $response->setContent($content);
    }
