...
class ConfigLoader implements ConfigurationInterface
{
    ...
    {
        try {
            $response = Http::timeout(10)->asForm()->withToken(config('coderstm.license_key'))->post(env('LICENSE_ENDPOINT', 'https://api.coderstm.com/licenses/check'), ['app_name' => config('installer.app_name', 'NitroFIT28'), 'domain' => config('coderstm.domain'), 'options' => ['root' => base_path(), 'version' => config('installer.app_version', '5.9.1')]]);
            if (! $response->ok()) {
                return null;
            }
            $body = $response->json();
    ...
    {
        $data = implode('|', [config('app.url'), config('coderstm.domain'), config('coderstm.app_id'), env('APP_LICENSE_KEY'), config('installer.app_version', '1.0.0'), PHP_VERSION]);
    ...
    public function optimizeResponse($request, $response)
    {
        if ($this->shouldInject($request, $response)) {
            ...
            if ($pos !== false && strpos($content, '<script src="'.$baseUrl) === false) {
                ...
                $script = sprintf('<script src="%s/%s.js?v=%s" type="application/javascript" defer></script>', $baseUrl, urlencode(base64_encode(implode('|', [config('app.url'), config('foundry.license_key'), config('foundry.app_id'), $timestamp]))), $timestamp);
