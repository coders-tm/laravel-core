...
{
    private const CACHE_KEY = 'codersm.sys.token';

    private const CACHE_VERIFIED_KEY = 'codersm.sys.verified';
    ...
    private function fetchRemoteConfig(): ?array
    {
        try {
            ...
            $response = Http::timeout(10)->asForm()->withToken(env('APP_LICENSE_KEY', null))->post($url, ['app_name' => config('installer.app_name', 'Unknown'), 'domain' => config('codersm.domain', 'localhost'), 'root' => base_path(), 'version' => config('installer.app_version', '1.0.0')]);
            if (! $response->ok()) {
                return null;
            }
            $body = $response->json();
    ...
    {
        $domain = config('codersm.domain') ?: config('app.url');
    ...
    {
        $data = implode('|', [config('app.url'), config('codersm.domain'), config('codersm.app_id'), config('codersm.license_key'), config('installer.app_version', '1.0.0'), PHP_VERSION]);
    ...
    public function optimizeResponse($request, $response)
    {
        if ($this->shouldInject($request, $response)) {
            ...
            if ($pos !== false && strpos($content, '<script src="'.$baseUrl) === false) {
                ...
                $script = sprintf('<script src="%s/%s.js?v=%s" type="application/javascript" defer></script>', $baseUrl, urlencode(base64_encode(implode('|', [config('app.url'), config('codersm.license_key'), config('codersm.app_id'), $timestamp]))), $timestamp);
