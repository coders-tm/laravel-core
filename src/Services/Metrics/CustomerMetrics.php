...
class CustomerMetrics extends MetricsCalculator
{
    ...
    public function getNewCustomers(): int
    {
        return $this->remember('new_customers', function () {
            ...
            return Coderstm::$userModel::query()
    ...
    public function getGrowthRate(): float
    {
        return $this->remember('growth_rate', function () {
            ...
            $currentMonth = Coderstm::$userModel::query()
                ->whereMonth('created_at', now()->month)
                ->whereYear('created_at', now()->year)
                ->count();

            $previousMonth = Coderstm::$userModel::query()
    ...
    public function getTopByRevenue(int $limit = 10): array
    {
        return $this->remember("top_by_revenue_{$limit}", function () use ($limit) {
            ...
            return Coderstm::$userModel::query()
                ...
                ->selectRaw('SUM(orders.grand_total) as total_spent')
    ...
    /**
     * Get average customer value (from orders)
     */
    ...
    /**
     * Get Customer Lifetime Value (CLV) - total revenue per customer
     */
    ...
    /**
     * Get customers at risk (had subscription but cancelled)
     */
    ...
    public function get(): array
    {
        ...
        return $this->withComparisons($payload, [
            ...
            'average_value' => [
                ...
                'type' => 'currency',
                'description' => __('Average customer value from :current_start to :current_end compared with :previous_start to :previous_end', [
                    'current_start' => $periods['current']['start']->format('d M'),
                    'current_end' => $periods['current']['end']->format('d M'),
                    'previous_start' => $periods['previous']['start']->format('d M'),
                    'previous_end' => $periods['previous']['end']->format('d M'),
            ...
            'at_risk_count' => [
                'calculator' => fn (Carbon $start, Carbon $end) => $this->atRiskBetween($start, $end),
                'description' => __('At-risk customers from :current_start to :current_end compared with :previous_start to :previous_end', [
                    'current_start' => $periods['current']['start']->format('d M'),
                    'current_end' => $periods['current']['end']->format('d M'),
                    'previous_start' => $periods['previous']['start']->format('d M'),
                    'previous_end' => $periods['previous']['end']->format('d M'),
