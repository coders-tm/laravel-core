...
/**
 * Discount Impact Report - Analyzes the impact of discounts on revenue.
 *
 * Generates period-based analysis of discount impact using a single efficient query
 * with period grouping via UNION ALL pattern.
 */
class DiscountImpactReport extends AbstractReport
{
    ...
    protected array $columns = [
        ...
        'orders_with_discount' => ['label' => 'Orders with Discount', 'type' => 'number'],
        'discount_rate' => ['label' => 'Discount Rate', 'type' => 'percentage'],
        'gross_revenue' => ['label' => 'Gross Revenue', 'type' => 'currency'],
        'total_discounts' => ['label' => 'Total Discounts', 'type' => 'currency'],
        'net_revenue' => ['label' => 'Net Revenue', 'type' => 'currency'],
        'avg_discount_per_order' => ['label' => 'Avg Discount/Order', 'type' => 'currency'],
        'discount_percentage_of_revenue' => ['label' => 'Discount % of Revenue', 'type' => 'percentage'],
    ];
    ...
    public function query(array $filters)
    {
        ...
        // Build period boundaries (label computed in toRow())
        $periodBoundaries = [];
        ...
        return DB::table(DB::raw("({$periodQuery->toSql()}) as periods"))
            ...
            ->select([
                ...
                DB::raw('COUNT(DISTINCT CASE WHEN discount_lines.id IS NOT NULL THEN orders.id END) as orders_with_discount'),
                DB::raw('COALESCE(SUM(orders.grand_total), 0) + COALESCE(SUM(discount_lines.value), 0) as gross_revenue'),
                DB::raw('COALESCE(SUM(discount_lines.value), 0) as total_discounts'),
                DB::raw('COALESCE(SUM(orders.grand_total), 0) as net_revenue'),
            ])
    ...
    public function toRow($row): array
    {
        ...
        $avgDiscountPerOrder = $ordersWithDiscount > 0
            ? $totalDiscounts / $ordersWithDiscount
            : 0;

        $discountPercentage = $grossRevenue > 0
            ? ($totalDiscounts / $grossRevenue) * 100
            : 0;
        ...
        return [
            ...
            'avg_discount_per_order' => (float) $avgDiscountPerOrder,
            'discount_percentage_of_revenue' => (float) $discountPercentage,
        ];
    }
    ...
    public function summarize(array $filters): array
    {
        ...
        return [
            'total_orders' => $totalOrders,
            'orders_with_discount' => $ordersWithDiscount,
            'total_gross_revenue' => format_amount($stats->gross_revenue),
            'total_discounts' => format_amount($stats->total_discounts),
            'total_net_revenue' => format_amount($stats->net_revenue),
