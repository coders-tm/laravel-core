...
trait ManagesSubscriptions
{
    public function newSubscription(string $type, $plan, string $billing = 'monthly', array $metadata = [])
    {
        ...
        $period = new Period($billingInterval, $billingIntervalCount, $billingStartDate);
        $subscription = new Coderstm::$subscriptionModel(['type' => $type, 'plan_id' => $plan->getKey(), 'status' => $isTrial ? SubscriptionStatus::TRIALING : SubscriptionStatus::PENDING, 'trial_ends_at' => $isTrial ? $trial->getEndDate() : null, 'starts_at' => $isTrial ? now() : $period->getStartDate(), 'expires_at' => $isTrial ? $trial->getEndDate() : $period->getEndDate(), 'billing_interval' => $billingInterval, 'billing_interval_count' => $billingIntervalCount, 'total_cycles' => $plan->contract_cycles, $this->getForeignKey() => $this->getKey()]);
        if ($billing === 'yearly') {
            $creditStartDate = $isTrial ? $billingStartDate : $subscription->starts_at ?? now();
            $creditPeriod = new Period($plan->interval->value, $plan->interval_count, $creditStartDate);
