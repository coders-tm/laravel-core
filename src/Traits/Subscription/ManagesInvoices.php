...
trait ManagesInvoices
{
    ...
    public function upcomingInvoice($start = false, $dateFrom = null, $plan = null, ?string $interval = null, ?string $couponCode = null): ?InvoiceRepository
    {
        ...
        $period = new Period($billingInterval, $billingIntervalCount, $dateFrom ?? $this->dateFrom());
        $dueDate = $start ? $this->dateFrom() : $this->expires_at;
        $dueDate = $dueDate && $dueDate->gt(now()) ? $dueDate : $period->getEndDate();
        $price = $billingInterval === 'year' ? (float) ($plan->yearly_fee ?? $plan->price) : (float) ($plan->price ?? 0);
        $discount = null;
        if ($couponCode) {
            ...
            if ($coupon && $coupon->canApplyToPlan($plan)) {
                ...
                $discount = ['type' => $discountType, 'value' => $coupon->value, 'description' => $coupon->name, 'coupon_id' => $coupon->id, 'coupon_code' => $coupon->promotion_code, 'auto_applied' => false];
            }
        } else {
        ...
        $lineItems = [['title' => $title, 'metadata' => ['description' => "{$fromDate} - {$toDate}", 'plan_id' => $plan->id], 'price' => $price, 'total' => $price, 'quantity' => 1, 'options' => ['title' => $title], 'discount' => $discount]];
        ...
        return new InvoiceRepository(['source' => 'Membership', 'customer_id' => $this->user?->id, 'orderable_id' => $this->id, 'orderable_type' => static::class, 'due_date' => $dueDate, 'billing_address' => $this->user?->billingAddress(), 'collect_tax' => true, 'line_items' => apply_filters('subscription.generate_line_items', $lineItems, $this)]);
    }
