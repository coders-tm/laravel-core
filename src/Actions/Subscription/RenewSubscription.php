...
class RenewSubscription
{
    public function execute($subscription, bool $charge = true)
    {
        ...
        if ($subscription->isContract()) {
            $contractPeriod = new Period($subscription->plan->interval->value, $subscription->plan->interval_count, $subscription->created_at ?? $subscription->starts_at);
        ...
        $subscription->fill(['starts_at' => $period->getStartDate(), 'expires_at' => $newExpiresAt, 'ends_at' => $graceEndsAt, 'trial_ends_at' => null])->save();
        ...
        if (! $isPaid && $invoice && config('coderstm.wallet.auto_charge_on_renewal', true) && $subscription->user) {
            try {
                ...
                logger()->error('Failed to charge wallet during subscription renewal', ['subscription_id' => $subscription->id, 'user_id' => $subscription->user_id, 'error' => $e->getMessage()]);
            }
        }
        if (! $isPaid && ! $graceEndsAt && $invoice && (float) $invoice->grand_total > 0) {
            $subscription->update(['status' => SubscriptionStatus::EXPIRED, 'ends_at' => null]);
    ...
    protected function executeNoCharge($subscription)
    {
        ...
        $startDate = $subscription->expires_at ?? Carbon::now();
        $period = new Period($subscription->getBillingInterval(), $subscription->getBillingIntervalCount(), $startDate);
        $isExpired = $subscription->expired();
        $subscription->fill(['status' => SubscriptionStatus::ACTIVE, 'expires_at' => $period->getEndDate(), 'ends_at' => null, 'trial_ends_at' => null, 'canceled_at' => null])->save();
        if ($isExpired) {
            $subscription->resetUsagesForRenewal();
            $subscription->advanceCreditResetsAt()->save();
        }
    ...
    protected function chargeFromWallet($subscription, $invoice): void
    {
        ...
        $transaction = $subscription->user->debitWallet(amount: $amount, source: 'subscription_renewal', description: "Subscription renewal - {$subscription->plan->label}", transactionable: $subscription, metadata: ['subscription_id' => $subscription->id, 'plan_id' => $subscription->plan_id, 'invoice_id' => $invoice->id, 'cycle' => $subscription->current_cycle]);
