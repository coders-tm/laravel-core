...
class FreezeSubscription
{
    public function execute($subscription, ?Carbon $releaseAt = null, ?string $reason = null, ?float $fee = null)
    {
        ...
        $subscription->fill(['status' => SubscriptionStatus::PAUSED, 'frozen_at' => now(), 'release_at' => $releaseAt])->save();
        if ($freezeFee > 0) {
        ...
        $subscription->logs()->create(['type' => LogType::UPDATED, 'message' => $logMessage]);
    ...
    protected function generateFreezeInvoice($subscription, float $fee, ?Carbon $releaseAt = null)
    {
        ...
        return $subscription->invoices()->create(['user_id' => $subscription->user_id, 'status' => 'open', 'description' => "Subscription freeze fee ({$period})", 'sub_total' => $fee, 'tax' => 0, 'total' => $fee, 'grand_total' => $fee, 'lines' => [['description' => "Freeze Fee - {$subscription->plan->label}", 'quantity' => 1, 'unit_amount' => $fee, 'amount' => $fee]]]);
    }
}
